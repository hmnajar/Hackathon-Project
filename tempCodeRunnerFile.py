...
playerStats = pd.read_csv('playerStats.csv', index_col="Player")
...
for player in teamPlayers[favoredTeam]:
    if player in playerStats.index:
        ...
        #favored_edited_CL = int(player_stats['CL%'].rstrip("%")) # just undo it all ill figure it out later i just wanna test what i got
...
team1EstScore = sum(favored_team_statlist[stat] * weights[stat] for stat in weights)
team2EstScore = sum(unfavored_team_statlist[stat] * weights[stat] for stat in weights)

team1EstScore += 30 # map win% bonus
